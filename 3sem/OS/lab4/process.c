...
static struct { tid_t tid; int status; bool used; } exit_table[MAX_PROCESSES];
...
process_wait (tid_t child_tid)
{
  ...
  /* Читаем код завершения из таблицы — память потока уже освобождена */
  int status = -1;
  lock_acquire (&exit_table_lock);
  for (int i = 0; i < MAX_PROCESSES; i++)
    if (exit_table[i].used && exit_table[i].tid == child_tid)
      {
        status = exit_table[i].status;
        exit_table[i].used = false; /* освобождаем слот */
        break;
      }
